...
def load_user(id):
    student_user = student_login.query.get(int(id))
    teacher_user = teacher_login.query.get(int(id))
    if student_user:
        return student_user
    elif teacher_user:
        return teacher_user
...
class ProfileForm(FlaskForm):
    ...
    first_name = StringField('First Name', validators=[DataRequired(),Length(min=2, max=10)],render_kw={"placeholder": "First Name"})
    last_name = StringField('Last Name', validators=[DataRequired(),Length(min=2, max=10)],render_kw={"placeholder": "Last Name"})
    department = StringField('Department',validators=[DataRequired(),Length(min=3,max=15)],render_kw={"placeholder":"Department"})
...
def student_profile():
    ...
    if form.validate_on_submit():
        user = students(FIRST_NAME=form.first_name.data, LAST_NAME=form.last_name.data, DEPARTMENT = form.department.data, PHONE_NUMBER = form.phone_number.data)
        if user:
            db.session.add(user)
            db.session.commit()
            return redirect(url_for('login'))
    return render_template('studentprofile.html',form=form)
...
def teacher_profile():
    ...
    if form.validate_on_submit():
        user = teachers(FIRST_NAME=form.first_name.data, LAST_NAME=form.last_name.data, DEPARTMENT = form.department.data, PHONE_NUMBER = form.phone_number.data)
        if user:
            db.session.add(user)
            db.session.commit()
            return redirect(url_for('login'))
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user:
            ...
            teacher = teacher_login.query.filter_by(EMAIL=form.email.data).first()
