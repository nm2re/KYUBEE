...
class ProfileForm(FlaskForm):
    ...
    department = StringField('Department', validators=[DataRequired(), Length(min=1, max=15)],
                             render_kw={"placeholder": "Department"})
    phone_number = StringField('Telephone', validators=[DataRequired(), Length(min=10, max=10)], render_kw={"placeholder": "Phone Number"})
...
def dashboard(user_type, user_id):
    ...
    if logged_user:
        flash(f"Current User Logged In: {logged_user.EMAIL}", 'error')
    else:
...
def teacher_profile(login_uuid):
    ...
    if form.validate_on_submit():
        db.session.query(teachers).filter_by(TEACHER_ID=login_uuid).update({"FIRST_NAME": form.first_name.data, "LAST_NAME": form.last_name.data,"DEPARTMENT_ID": form.department.data, "PHONE_NUMBER": form.phone_number.data})
